...
import io.zhijun.spring.annotation.AnnotationUtils;
...
public class AnnotatedInjectionBeanPostProcessor implements SmartInstantiationAwareBeanPostProcessor,
    ...
    /**
     * The property name of metadata cache size : "rose.spring.injection.metadata.cache.size"
     */
    public static final String CACHE_SIZE_PROPERTY_NAME = ROSE_SPRING_PROPERTY_NAME_PREFIX + "injection.metadata.cache.size";
    ...
    /**
     * The property name of metadata cache expire time : "rose.spring.injection.metadata.cache.expire.time"
     */
...
//     )
    ...
    public AnnotatedInjectionBeanPostProcessor(Class<? extends Annotation> annotationType, Class<? extends Annotation>... otherAnnotationTypes) {
        ...
        List<Class<? extends Annotation>> types = new java.util.ArrayList<>(otherAnnotationTypes.length + 1);
        types.add(annotationType);
        java.util.Collections.addAll(types, otherAnnotationTypes);
        ...
        if (candidateConstructors == null) {
            ...
            synchronized (this.candidateConstructorsCache) {
                ...
                if (candidateConstructors == null) {
                    ...
                    List<Constructor<?>> candidates = new java.util.ArrayList<>(rawCandidates.length);
                    ...
                    for (Constructor<?> candidate : rawCandidates) {
                        ...
                        if (ann != null) {
                            if (requiredConstructor != null) {
                                String message = java.lang.String.format("Invalid injection constructors: {}. "
    ...
    protected List<AnnotatedFieldElement> findFieldAnnotationMetadata(final Class<?> beanClass) {
        ...
        final List<AnnotatedFieldElement> elements = new java.util.LinkedList<>();
    ...
    protected List<AnnotatedMethodElement> findAnnotatedMethodMetadata(final Class<?> beanClass) {
        ...
        final List<AnnotatedMethodElement> elements = new java.util.ArrayList<>();
    ...
    protected final AnnotationAttributes doGetAnnotationAttributes(AnnotatedElement annotatedElement, Class<? extends Annotation> annotationType) {
        // Using Spring core directly
        Annotation annotation = annotatedElement.getAnnotation(annotationType);
        if (annotation == null) {
            return null;
        }
        return org.springframework.core.annotation.AnnotationUtils.getAnnotationAttributes(annotation, classValuesAsString, nestedAnnotationsAsMap);
    }
    ...
    private InjectionMetadata buildAnnotatedMetadata(final Class<?> beanClass) {
        ...
        List<InjectedElement> elements = new java.util.ArrayList<>(fieldElements.size() + methodElements.size());
    ...
    public void afterPropertiesSet() {
        this.candidateConstructorsCache = new java.util.concurrent.ConcurrentHashMap<>(cacheSize);
        this.injectionMetadataCache = new java.util.concurrent.ConcurrentHashMap<>(cacheSize);
    }
