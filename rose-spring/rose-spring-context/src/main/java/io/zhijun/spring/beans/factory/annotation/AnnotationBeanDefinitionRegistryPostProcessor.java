...
import io.zhijun.spring.context.annotation.ExposingClassPathBeanDefinitionScanner;
...
import static io.zhijun.spring.context.annotation.AnnotatedBeanDefinitionRegistryUtils.resolveAnnotatedBeanNameGenerator;
import static io.zhijun.spring.annotation.AnnotationUtils.tryGetMergedAnnotation;
...
public abstract class AnnotationBeanDefinitionRegistryPostProcessor implements BeanDefinitionRegistryPostProcessor,
    ...
    public AnnotationBeanDefinitionRegistryPostProcessor(Class<? extends Annotation> annotationType,
        ...
        this.supportedAnnotationTypes = new java.util.LinkedHashSet<Class<? extends Annotation>>();
        this.packagesToScan = new java.util.LinkedHashSet<>(); packagesToScan.forEach(this.packagesToScan::add);
        addSupportedAnnotationType(annotationType);
    }

    public void addSupportedAnnotationType(Class<? extends Annotation>... annotationTypes) {
        if (annotationTypes == null || annotationTypes.length < 1) throw new IllegalArgumentException("The argument of annotation types can.t be empty");
        for (Class<? extends Annotation> type : annotationTypes) { if (type == null) throw new IllegalArgumentException("Any element of annotation types can.t be null"); }
        java.util.Collections.addAll(this.supportedAnnotationTypes, annotationTypes);
    }
    ...
    protected static AnnotationAttributes getAnnotation(AnnotatedElement annotatedElement, Class<? extends Annotation> annotationType) {
    ...
    protected Map<String, AnnotatedBeanDefinition> registerMainBeanDefinitions(ExposingClassPathBeanDefinitionScanner scanner,
        ...
        Map<String, AnnotatedBeanDefinition> mainBeanDefinitions = new LinkedHashMap<>();
    ...
    void putBeanDefinitions(Map<String, AnnotatedBeanDefinition> mainBeanDefinitions,
        ...
        if (beanDefinition instanceof AnnotatedBeanDefinition) {
            putBeanDefinition(mainBeanDefinitions, (AnnotatedBeanDefinition) beanDefinition, beanDefinitionHolder.getBeanName());
            putBeanDefinition(mainBeanDefinitions, (AnnotatedBeanDefinition) beanDefinition, beanDefinitionHolder.getAliases());
        } else {
        ...
        if (isEmpty(beanDefinitionHolders)) {
            if (logger.isWarnEnabled()) {
                logger.warn("No Spring Bean annotation @{} was found under base packages : {}",
                        getSupportedAnnotationTypeNames(), java.util.Arrays.toString(basePackages));
            }
        } else {
            if (logger.isInfoEnabled()) {
                logger.info("{} annotations {} components { {} } were scanned under packages : {}", beanDefinitionHolders.size(),
                        getSupportedAnnotationTypeNames(), beanDefinitionHolders, java.util.Arrays.toString(basePackages));
            }
        }
    }
