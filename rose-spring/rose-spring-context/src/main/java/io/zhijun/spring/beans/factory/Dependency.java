...
public class Dependency {
    ...
    final List<Dependency> children = new java.util.LinkedList<>();
    ...
    public Dependency addChildren(String... childBeanNames) {
        return this.addChildren(java.util.Arrays.asList(childBeanNames));
    }
    ...
    private static List<Dependency> createList(Iterable<String> beanNames) {
        int length = 0; for (String ignored : beanNames) { length++; }
        if (length < 1) {
            return emptyList();
        }
        List<Dependency> dependencies = new java.util.ArrayList<>(length);
