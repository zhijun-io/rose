...
import io.zhijun.spring.config.ConfigurationProperty;
...
public class ConfigurationPropertyRepository implements EnvironmentAware, InitializingBean, DisposableBean {
    ...
    public static final String PROPERTY_NAME_PREFIX = ROSE_SPRING_PROPERTY_NAME_PREFIX + "config-property-repository.";
     ...
     * <pre>{@code
     *   ConfigurationPropertyRepository repository = new ConfigurationPropertyRepository();
     *   // Assume the environment has "rose.spring.config-property-repository.max-size=3"
     *   repository.setEnvironment(environment);
     *   repository.afterPropertiesSet();
    ...
    public void afterPropertiesSet() {
        this.repository = new java.util.concurrent.ConcurrentHashMap<>(this.maxSize);
    }
    ...
    private void assertMaxSize() {
        if (repository != null && repository.size() >= maxSize) {
            String message = String.format("The size of repository is greater than max size : {}. " +
