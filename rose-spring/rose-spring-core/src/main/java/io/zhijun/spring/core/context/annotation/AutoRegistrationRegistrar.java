    ...
    protected void registerBeanDefinitions(AnnotationMetadata metadata,
        ...
        for (AutoRegistrationBean bean : beans) {
            if (!bean.isAutoRegistered(environment)) {
                log.debug("Auto-registration bean [{}] disabled, skipped", bean.getBeanName());
                continue;
            }
            String beanName = bean.getBeanName();
            if (registry.containsBeanDefinition(beanName)) {
                log.debug("Bean [{}] already registered, skipped", beanName);
                continue;
            }
            ...
            log.info("Auto-registered bean [{}] of type [{}]", beanName, bean.getClass().getName());
        }
    }
}
