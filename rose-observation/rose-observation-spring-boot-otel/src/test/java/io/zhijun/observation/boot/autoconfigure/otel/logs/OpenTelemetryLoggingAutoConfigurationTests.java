...
class OpenTelemetryLoggingAutoConfigurationTests {
    ...
    void batchProcessorNotCreatedWithoutExporter() {
        contextRunner.run(context -> assertThat(context).doesNotHaveBean(BatchLogRecordProcessor.class));
    }
    ...
    void customLoggerProviderBuilderCustomizerApplied() {
        contextRunner
                .withUserConfiguration(
                        DefaultLogRecordExporterConfiguration.class, CustomLoggerProviderConfiguration.class)
                .run(context -> {
                    assertThat(context).hasSingleBean(SdkLoggerProvider.class);
    ...
    void customLogRecordProcessorTakesPrecedence() {
        contextRunner
                .withUserConfiguration(
                        DefaultLogRecordExporterConfiguration.class, CustomLogRecordProcessorConfiguration.class)
                .run(context -> {
                    assertThat(context).hasSingleBean(LogRecordProcessor.class);
                    assertThat(context.getBean(LogRecordProcessor.class))
                            .isSameAs(context.getBean(CustomLogRecordProcessorConfiguration.class)
                                    .customLogRecordProcessor());
                });
    }
