...
/**
 * Unit test for {@link OnOpenTelemetryLoggingCondition}.
 */
class OnOpenTelemetryLoggingConditionTests {
    ...
    void shouldMatchWhenLoggingIsEnabled() {
        environment.setProperty("rose.otel.logs.enabled", "true");
        when(context.getEnvironment()).thenReturn(environment);

        ConditionOutcome outcome = condition.getMatchOutcome(context, metadata);

        assertThat(outcome.isMatch()).isTrue();
        assertThat(outcome.getMessage()).contains("rose.otel.logs.enabled is true");
    }

    @Test
    void shouldNotMatchWhenLoggingIsDisabled() {
        environment.setProperty("rose.otel.logs.enabled", "false");
        when(context.getEnvironment()).thenReturn(environment);

        ConditionOutcome outcome = condition.getMatchOutcome(context, metadata);

        assertThat(outcome.isMatch()).isFalse();
        assertThat(outcome.getMessage()).contains("rose.otel.logs.enabled is false");
    }
    ...
    void shouldMatchByDefaultWhenPropertyIsNotSet() {
        ...
        assertThat(outcome.isMatch()).isTrue();
        assertThat(outcome.getMessage()).contains("OpenTelemetry logging is enabled by default");
    }
}
