...
class ObservationAutoConfigurationTests {
    ...
    void noFailureWithSingleConventionsProvider() {
        contextRunner
                .withBean("provider", AiObservationConventionsProvider.class,
                        () -> AiObservationConventionsProvider.of("openinference"))
                .run(context ->
                        assertThat(context).hasNotFailed());
    }
    ...
    void failsWithMultipleConventionsProviders() {
        contextRunner
                .withBean("provider1", AiObservationConventionsProvider.class,
                        () -> AiObservationConventionsProvider.of("openinference"))
                .withBean("provider2", AiObservationConventionsProvider.class,
                        () -> AiObservationConventionsProvider.of("opentelemetry"))
                .run(context -> {
