...
| Artifact | 包名 | 说明 |
|----------|------|------|
| `rose-annotation` | `io.zhijun.annotation` | 零依赖契约注解（含 `@RosePropertyHint`） |
| `rose-core` | `io.zhijun.core` | 框架无关工具 |
| `rose-annotation-processor` | `io.zhijun.annotation.processor` | `RoseSinceProcessor` / `RoseInternalApiProcessor` / `RosePropertyMetadataProcessor` |
