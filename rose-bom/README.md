...
| `rose-xxx-spring-boot-{slice}` | 可选 Boot slice（仍属 Boot 层，非 `core`/`spring`） | `rose-devservice-spring-boot-postgresql`、`rose-observation-spring-boot-otel` |
...
**分层优先**：Maven 模块管发布边界，包名管逻辑分层；业务域包名**不重复** `core` 段（artifact 已是 `rose-xxx-core`）。

| 层级 | 规则 | 示例 |
|------|------|------|
| 平台 Spring | `rose-spring-*` → `io.zhijun.spring.*` | `io.zhijun.spring.core.env.*` |
| 平台 Boot | `rose-spring-boot-*` → `io.zhijun.boot.*` | `io.zhijun.boot.env.defaults.*` |
| 业务域 API | `rose-xxx-core` → `io.zhijun.{domain}.core.*` | `io.zhijun.multitenancy.core.context.*` |
| Spring 集成 | `rose-xxx-spring` → `io.zhijun.{domain}.spring.*` | `io.zhijun.multitenancy.spring.web.filter.*` |
| Boot 装配 | `rose-xxx-spring-boot` → `io.zhijun.{domain}.boot.autoconfigure.*` | `io.zhijun.multitenancy.boot.autoconfigure.*` |
| Boot slice | `rose-xxx-spring-boot-{slice}` → `io.zhijun.{domain}.boot.autoconfigure.{slice}.*` | `io.zhijun.devservice.boot.autoconfigure.postgresql.*` |
| Boot 注册 | `rose-xxx-spring-boot`（DevService 等） | `io.zhijun.devservice.boot.registration.*` |
...
| `rose-observation-spring-boot` | 域级 Boot 入口，聚合 OTel slice | 与 `rose-multitenancy-spring-boot` 同级；slice 命名见 `rose-observation-spring-boot-{otel,logback,...}` |

### Base

| Artifact | 说明 |
|----------|------|
| `rose-annotation` | 纯 Java 契约注解（`io.zhijun.annotation`） |
| `rose-core` | 纯 Java 工具（`io.zhijun.core`）；传递 `rose-annotation` |
| `rose-test` | 跨主题 JUnit 工具（`test` scope） |
| `rose-annotation-processor` | 编译期注解处理（`optional`） |
...
| Artifact | 说明 |
|----------|------|
| `rose-observation-core` | `TelemetryConventionsBackend` API（无 Boot） |
| `rose-observation-spring-boot` | 默认 OTel 栈 + conventions 选型 Boot 自动装配 |
| `rose-observation-spring-boot-otel` | OTel SDK Boot 自动配置（仅 SDK） |
| `rose-observation-spring-boot-logback` | 日志桥接 |
| `rose-observation-spring-boot-micrometer-otlp` | Micrometer → OTLP（默认指标路径） |
| `rose-observation-spring-boot-micrometer-bridge` | Micrometer → OTel SDK |
| `rose-observation-spring-boot-conventions-otel` | 语义约定 |
...
| Artifact | 说明 |
|----------|------|
| `rose-devservice-core` | API、`BootstrapMode`、Testcontainers 工具（包 `io.zhijun.devservice.core.{api,bootstrap,container,docker,util}.*`） |
| `rose-devservice-spring-boot` | Boot 自动配置（`boot.autoconfigure`）与容器注册（`boot.registration`） |
...
| `rose-spring-web` / `rose-spring-boot-web` | Reactor 脚手架（仅 `package-info`）；实现完成后再加入 BOM |
