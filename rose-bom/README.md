...
| 连接器 slice | `rose-devservice-spring-boot-postgresql` → `io.zhijun.devservice.postgresql.*`；`rose-devservice-spring-boot-otel` → `io.zhijun.devservice.otel.*` |
...
| `rose-observability-spring-boot` | 域级 Boot 入口，聚合 OTel slice | 与 `rose-multitenancy-spring-boot` 同级；slice 命名见 `rose-observability-spring-boot-{otel,logback,...}` |
...
| Artifact | 说明 |
|----------|------|
| `rose-observability-core` | `TelemetryConventionsBackend` API（无 Boot） |
| `rose-observability-spring-boot` | 默认 OTel 栈 + conventions 选型 Boot 自动装配 |
| `rose-observability-spring-boot-otel` | OTel SDK Boot 自动配置（仅 SDK） |
| `rose-observability-spring-boot-logback` | 日志桥接 |
| `rose-observability-spring-boot-micrometer-otlp` | Micrometer → OTLP（默认指标路径） |
| `rose-observability-spring-boot-micrometer-bridge` | Micrometer → OTel SDK |
| `rose-observability-spring-boot-conventions-otel` | 语义约定 |
...
| Artifact | 说明 |
|----------|------|
| `rose-devservice-core` | API、`BootstrapMode`、Testcontainers 工具（包 `io.zhijun.devservice.core.*`） |
