...
| Boot 装配 | `rose-xxx-spring-boot` → `io.zhijun.{domain}.autoconfigure.*` | `io.zhijun.multitenancy.autoconfigure.*` |
| Boot slice | `rose-xxx-spring-boot-{slice}` → `io.zhijun.{domain}.autoconfigure.{slice}.*` | `io.zhijun.devservice.autoconfigure.postgresql.*` |
