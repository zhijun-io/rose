...
| 业务域 API | `rose-xxx-core` → `io.zhijun.{domain}.{feature}.*` | `io.zhijun.multitenancy.context.*` |
| Spring 集成 | `rose-xxx-spring` → `io.zhijun.{domain}.spring.*` | `io.zhijun.multitenancy.spring.web.filter.*` |
| Boot 装配 | `rose-xxx-spring-boot` → `io.zhijun.{domain}.autoconfigure.*` | `io.zhijun.multitenancy.boot.autoconfigure.*` |
| Boot slice | `rose-xxx-spring-boot-{slice}` → `io.zhijun.{domain}.autoconfigure.{slice}.*` | `io.zhijun.devservice.boot.autoconfigure.postgresql.*` |
