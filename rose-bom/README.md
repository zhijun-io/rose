...
| 域缩写 | `devservice` → `io.zhijun.dev.*`（与 artifact 不完全同名，属既定缩写） |
| Boot 自动配置 | 可放在 `*-spring-boot` 模块，包名仍可用 `{domain}.core.autoconfigure.*`（见 multitenancy） |
| 连接器 slice | `rose-devservice-spring-boot-postgresql` → `io.zhijun.dev.postgresql.*` |
...
| `rose-opentelemetry-core` | 兼容坐标，传递依赖 `rose-opentelemetry-spring-boot` | 保留旧 artifact，Boot 装配已迁入 `-spring-boot` |
...
| `rose-opentelemetry-core` | 兼容坐标（传递 `rose-opentelemetry-spring-boot`） |
| `rose-opentelemetry-spring-boot` | OTel SDK Boot 自动配置（traces / metrics / logs / resource / exporter） |
...
| Artifact | 说明 |
|----------|------|
| `rose-devservice-core` | API、`BootstrapMode`、Testcontainers 工具（包 `io.zhijun.dev.*`） |
