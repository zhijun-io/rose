...
- **Package layout (breaking)**: business domains mirror Maven modules — `{domain}.core.*` (`*-core`), `{domain}.spring.*` (`*-spring`), `{domain}.boot.autoconfigure[.{slice}].*` (`*-spring-boot`); DevService registration at `{domain}.boot.registration.*`. See `docs/rose-conventions.md`.
...
- Configuration IDE metadata for `@ConfigurationProperties` is generated by Spring Boot `spring-boot-configuration-processor` (field Javadoc + defaults); removed Rose `@PropertyHint` / `PropertyMetadataProcessor`.
- Dev Service Testcontainers wrappers dropped `Rose` class prefix (e.g. `PostgresqlContainer`); see `docs/rose-conventions.md` §4.
- AutoConfiguration classes now register through both `META-INF/spring.factories` and `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports` (dual registration) to prepare for the Spring Boot 3 migration; behavior unchanged, `spring.factories` retained.
...
- `README.md`, `docs/rose-conventions.md`, `docs/design/`, `wiki/`
