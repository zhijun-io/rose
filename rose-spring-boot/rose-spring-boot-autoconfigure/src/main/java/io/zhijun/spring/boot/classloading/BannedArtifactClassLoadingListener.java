...
public class BannedArtifactClassLoadingListener extends SpringApplicationRunListenerAdapter implements Ordered {
    ...
    /**
     * 系统属性名：逗号分隔的被禁构件列表，格式 {@code groupId:artifactId}。
     * 支持 {@code *} 通配符，例如 {@code com.example:*} 匹配所有 com.example 组的构件，
     * {@code *:bad-lib} 匹配任意组内 artifactId 为 bad-lib 的构件。
     */
    public static final String BANNED_ARTIFACTS_PROPERTY_NAME =
            ROSE_SPRING_BOOT_PROPERTY_NAME_PREFIX + "banned-artifacts";
    ...
    private void banArtifacts() {
        ...
        List<Artifact> detectedArtifacts = detector.detect(false);
