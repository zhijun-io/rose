...
启动时扫描 `classpath*:config/default/*.properties`、`*.yml`、`*.yaml`，扁平化后合并进 Spring Boot `defaultProperties`（最低优先级，`application.yml` 可覆盖）。
...
| `rose-spring-boot-actuator` | `actuator.properties` | —（`rose.actuator.task-scheduler.*` 推荐默认） |
...
| `rose.actuator.task-scheduler.pool-size` | `1` | 线程池大小（见 `config/default/actuator.properties`） |
