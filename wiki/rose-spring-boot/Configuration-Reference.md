...
Default Boot auto-configuration exclusions ship in `META-INF/config/default/*.properties` from
 `rose-spring-boot-autoconfigure`, and
...
|-------------------------------------------|------------------|--------------------------------------------------------------------------|------------------|
 | `rose-spring-boot-autoconfigure`          | runtime          | Boot startup SPI, default config merge, diagnostics, shared task support, baseline auto-configuration | Rarely direct    |
...
- `*-autoconfigure` modules are for manual composition or framework-internal aggregation.
 - `rose-spring-boot-autoconfigure` is shared infrastructure and should usually not be used as the application entrypoint.
...
|----------------------------------------------|---------|---------------------------------|----------------------------------------------------------------------|
 | `rose.default-config.enabled`                | `true`  | `rose-spring-boot-autoconfigure`         | Merge `config/default/*` and `META-INF/config/default/*` into Boot defaults |
 | `rose.default-config.locations`              | empty   | `rose-spring-boot-autoconfigure`         | Append extra classpath patterns to the default config merge scan     |
| `rose.autoconfigure.exclude`                 | empty   | `rose-spring-boot-autoconfigure` | Accumulate additional Boot auto-configuration exclusions across property sources |
 | `rose.diagnostics.artifacts-collision.enabled` | `false` | `rose-spring-boot-autoconfigure`       | Detect duplicate Maven coordinates on startup and fail fast          |
