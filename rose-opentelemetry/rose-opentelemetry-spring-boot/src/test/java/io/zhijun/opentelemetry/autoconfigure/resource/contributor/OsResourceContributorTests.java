...
class OsResourceContributorTests {
    ...
    @Test
    void shouldContributeOsTypeForAix() {
        when(osInfo.getName()).thenReturn("AIX 7.2");

        contributor.contribute(resourceBuilder);

        verify(resourceBuilder).put(OsResourceContributor.OS_TYPE, "aix");
    }

    @Test
    void shouldContributeOsTypeForFreeBsd() {
        when(osInfo.getName()).thenReturn("FreeBSD");
