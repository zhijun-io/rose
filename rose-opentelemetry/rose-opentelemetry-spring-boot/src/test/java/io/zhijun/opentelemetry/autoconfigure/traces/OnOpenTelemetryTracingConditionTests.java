package io.zhijun.opentelemetry.autoconfigure.traces;
...
class OnOpenTelemetryTracingConditionTests {
    ...
    void shouldMatchWhenPropertyIsEnabled() {
        environment.setProperty("rose.otel.traces.enabled", "true");
    ...
    void shouldNotMatchWhenPropertyIsDisabled() {
        environment.setProperty("rose.otel.traces.enabled", "false");
        when(context.getEnvironment()).thenReturn(environment);
