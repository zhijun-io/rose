...
| Core | `rose-opentelemetry-core` | 兼容坐标（传递 `rose-opentelemetry-spring-boot`） |
...
按需追加 bridge slice（logback、micrometer-registry-otlp 等）。`rose-opentelemetry-core` 仍可用，传递依赖 `-spring-boot`。
