...
| Boot | `rose-opentelemetry-spring-boot` | Actuator + OTLP runtime 聚合入口 |
...
Requires `rose-spring-boot-core` on the classpath for `config/default/*` / `META-INF/config/default/*` loading and `ConfigurableAutoConfigurationImportFilter`. The Boot module (`rose-opentelemetry-spring-boot`) includes it transitively via `rose-spring-boot-core`.
