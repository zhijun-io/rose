...
| 模块 | Artifact | 说明 |
|------|----------|------|
| Boot | `rose-opentelemetry-spring-boot` | SDK Boot 自动配置（traces / metrics / logs / resource / exporter） |
| Core | `rose-opentelemetry-core` | **默认栈聚合**（`-spring-boot` + logback + OTLP metrics + semantic conventions + Actuator） |
...
- OTel SDK Spring Boot 自动配置（`rose-opentelemetry-spring-boot`）
- 默认栈聚合（`rose-opentelemetry-core`）
- Logback bridge
...
Requires `rose-spring-boot-core` on the classpath for `config/default/*` / `META-INF/config/default/*` loading and `ConfigurableAutoConfigurationImportFilter`. The Boot module includes it transitively.
...
按需追加 `rose-opentelemetry-logback-bridge`、`rose-opentelemetry-micrometer-registry-otlp` 等 slice。
