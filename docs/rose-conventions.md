...
实现细节、非稳定 API 放在 `*.internal.*` 子包；公共 SPI 与 `@ConfigurationProperties` 留在对外包。与 `@Incubating` / `InternalApiProcessor` 互补：
...
- 渐进归位示例：`PropertySourceDiffSupport` → `env.event.internal`；`ClasspathMavenArtifactScanner` → `boot.diagnostics.internal`。
...
- **公共测试工具**（`rose-test`）：`ApplicationContextTestSupport.autoConfiguration(...)` 用于 Boot auto-config slice 测试；`logback-test.xml` 统一测试日志。域专属基类（如 `rose-devservice-test`）不迁入 `rose-test`。
...
- `maven-compiler-plugin`：**main** 源码 `-parameters`、`-Xlint:deprecation`、`-Xlint:-processing`、`-Werror`；**test** 源码仅 `-parameters` 与 `-Xlint:-processing`（弃用告警不阻断测试编译）。
- `spotless-maven-plugin` + `palantir-java-format`（`verify` 阶段 `spotless:check`；**仅当构建 JDK ≥ 11** 时通过 `spotless-check` profile 激活，因插件与 formatter 需 JDK 11+）。本地：`./mvnw spotless:apply`（须 JDK 11+）；Java 8 仅作编译目标时照常 `mvn verify`，不跑格式化门禁。
