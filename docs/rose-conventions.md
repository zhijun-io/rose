...
notBlank(s, "message");
BaseDevServiceProperties.

isFixedPort(port);
ClassUtils.

isPresent("com.example.Foo",classLoader);
```
...
- 新增 `AutoConfiguration` 时两处一起改；非自动配置条目（`EnvironmentPostProcessor`、`FailureAnalyzer` 等）仍只写在
  `spring.factories`。

### `internal` 包边界

实现细节、非稳定 API 放在 `*.internal.*` 子包；公共 SPI 与 `@ConfigurationProperties` 留在对外包。与 `@Incubating` /
`InternalApiProcessor` 互补：

- 包名一眼可辨非公共 API（借鉴 sdk-java `io/temporal/internal/*`）；包路径中含 `.internal` 段即视为内部包。
- `InternalApiProcessor` 对 `@Internal` 类型与 `*.internal.*` 包内类型均在 public API 泄漏时告警。
- 渐进归位示例：`PropertySourceDiffSupport` → `env.event.internal`；`ClasspathMavenArtifactScanner` →
  `boot.diagnostics.internal`。

### 依赖兼容下界（BOM 对齐版本）

Rose 构建锁 **Spring Boot 2.7.18** / **Java 8+**。版本属性注释见 `rose-parent` / `rose-build` POM；与 `renovate.json` 一致：

| 依赖                           | Rose 锁定                            | 兼容下界 / 约束                                                                                                                          |
|------------------------------|------------------------------------|------------------------------------------------------------------------------------------------------------------------------------|
| Spring Boot                  | 2.7.18                             | `[2.7.0, 3.0.0)`；Reactor 不验证 SB3                                                                                                   |
...
| OpenTelemetry SDK            | `${opentelemetry.version}`         | 随 `opentelemetry-bom`；须与 `opentelemetry-instrumentation` BOM 配套升级                                                                  |
| OpenTelemetry semconv        | `${opentelemetry-semconv.version}` | 随 SDK 大版本对齐                                                                                                                        |
| MyBatis-Plus                 | `${mybatis-plus.version}`          | 随 `mybatis-plus-bom`；勿与 SB2.7 传递依赖 major 冲突                                                                                        |
| Micrometer / Logback / SLF4J | SB2.7 BOM 传递                       | Renovate 禁止 major 跃迁（见 `renovate.json` `allowedVersions`）                                                                          |
| Kotlin stdlib                | `${kotlin.version}`                | 仅 OTel exporter 测试/运行时；非 Rose 公共 API                                                                                               |

下游若自行覆盖版本，须自行验证与 SB2.7 的传递依赖是否冲突。

### Starter / 聚合模块

`-spring-boot` 模块承载 `AutoConfiguration`；**纯依赖聚合**模块不得写业务逻辑。

| 模块                             | 角色                                            | 结论                       |
|--------------------------------|-----------------------------------------------|--------------------------|
| `rose-*-spring-boot-{slice}`   | 各 slice 的 `AutoConfiguration`                 | 符合（非 starter 聚合）         |
| `rose-observation-spring-boot` | 域级装配 + `ConventionsSelectorAutoConfiguration` | 符合（有 autoconfigure 职责）   |
| `rose-devservice-spring-boot`  | DevService 核心 autoconfigure                   | 符合                       |
| `rose-bom`                     | 消费者 BOM                                       | 仅 `dependencyManagement` |

域专属测试基类放 `rose-devservice-test` 等 `-test` 模块；跨域 Boot slice 测试用 `rose-test`（见 §6）。
...
- **公共测试工具**（`rose-test`）：`ApplicationContextTestSupport.autoConfiguration(...)` 用于 Boot auto-config slice 测试；
  `logback-test.xml` 统一测试日志。域专属基类（如 `rose-devservice-test`）不迁入 `rose-test`。

构建 profile 与 CI
详见 [wiki/rose-build/Profiles-Management.md](../wiki/rose-build/Profiles-Management.md)、[wiki/rose-build/CI-CD-Integration.md](../wiki/rose-build/CI-CD-Integration.md)。

## 7. 编译与静态检查

- `maven-compiler-plugin`：**main** 源码 `-parameters`、`-Xlint:deprecation`、`-Xlint:-processing`、`-Werror`；**test** 源码仅
  `-parameters` 与 `-Xlint:-processing`（弃用告警不阻断测试编译）。
- Error Prone 门禁见 [inspiration-sdk-java.md](inspiration-sdk-java.md) 第二批规划。

## 8. 提交流程

- **Conventional Commits**：`feat:`、`fix:`、`docs:`、`chore:`、`refactor:` 等。
- **一 PR 一主题**；依赖升级由 Renovate 处理。
- 应用不继承 `rose-parent` / `rose-build`；消费方 `import rose-bom`
  （见 [wiki/rose-bom/Consumer-Guide.md](../wiki/rose-bom/Consumer-Guide.md)）。
...
BOM
托管坐标完整列表见 [wiki/rose-bom/Consumer-Guide.md § BOM artifacts](../wiki/rose-bom/Consumer-Guide.md#bom-artifacts)（与
`rose-bom/pom.xml` 同步）。
