...
> **配套：** [property-source 设计](./rose-spring-property-source-design.md)（同 artifact，包 `io.zhijun.spring.core.propertysource.*`）  
...
在 `rose-spring-core` 内为 Spring `Environment` 增加**可观测**与**可响应**能力：任何来源（Boot、Cloud、property-source 文件热更、业务代码）对 `PropertySources` 的增删改，都能以统一事件模型通知下游。
...
`I18nCloudRefreshAdapter`（或等价物）监听 `EnvironmentChangeEvent`，将 `event.getKeys()` 转发给 `PropertySourcesRefreshEnvironmentListener.onEnvironmentChangeKeys`。  
