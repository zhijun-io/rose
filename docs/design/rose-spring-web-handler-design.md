...
| 项           | 状态                                 |
|-------------|------------------------------------|
| 模块 / 代码     | 🚧 脚手架（`package-info`）；实现待 Phase 1 |
| Phase 1 MVC | 规划                                 |

> **关联：
** [bootstrap-diagnostics §7](./rose-spring-boot-bootstrap-diagnostics-design.md#7-启动顺序与-initializer-协作)、
`rose-multitenancy-spring`、`rose-opentelemetry`
...
```bash
mvn -pl rose-spring/rose-spring-web,rose-spring-boot/rose-spring-boot-web test
```
...
rose-spring/
  rose-spring-core
  rose-spring-web/                 # Servlet MVC：registry + handler SPI（无 Boot）
...
| **1c** | `rose-spring-boot-web`：Actuator `RoseWebEndpointsEndpoint`（optional） |
...
Filter / `HandlerInterceptor` **无法** 在参数解析前后插入 `HandlerMethod` 级钩子。Phase 1 采用 **`BeanPostProcessor`
装饰 `RequestMappingHandlerAdapter`**（不替换 `@Primary` bean）：
...
- **唯一 BPP**：`rose-spring-web` 内仅一个 PostProcessor；第三方不得再包装同一 adapter（文档约定）。
...
**`preHandle` 返回 false：** 抛出 `HandlerMethodAbortedException`；`RoseInvocableHandlerMethod` 捕获后由
`@ControllerAdvice` 或默认 **403 Forbidden**（`rose.web.handler.abort-status`，默认 403）。

**Argument 链：** 自定义 `HandlerMethodArgumentResolverComposite` 装饰器，在每个 delegate resolve 前后调用
...
| 类                                     | 模块                         | 作用                              |
|---------------------------------------|----------------------------|---------------------------------|
| `TenantHandlerMethodInterceptor`      | `rose-multitenancy-spring` | 方法级 `@TenantRequired`           |
...
### 7.1 `rose-spring-boot-web`

```properties
org.springframework.boot.autoconfigure.EnableAutoConfiguration=\
io.zhijun.boot.web.autoconfigure.RoseSpringWebAutoConfiguration
```
...
2. Phase 1a registry（`rose-spring-web`）→ 1b interceptor → 1c `rose-spring-boot-web`
3. 文档：本规格 + 主题 README
